...
powershell -Command ^
"(Get-Content '%REPO_FILE%') -replace '\"sha256\"\s*:\s*\"[a-f0-9]*\"', '\"sha256\": \"%PKG_HASH%\"' | Set-Content '%REPO_FILE%'"
...
if /i "%DOZIP%"=="y" (
    ...
    powershell -Command ^
    "(Get-Content '%PKG_FILE%') -replace '\"download_sha256\"\s*:\s*\"[a-f0-9]*\"', '\"download_sha256\": \"%ZIP_HASH%\"' | Set-Content '%PKG_FILE%'"
)
